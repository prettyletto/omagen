...
func writeHyprlandWithAnimationsAndShell(themeDir string, p Palette, borderStyle string, borderSize int, shape, spacing, depth, active, inactive string, borderSpeed int, animations session.AnimationsStyle, windowOpacity *int, shellPreset string, barSpecs ...*bar.BarSpec) error {
	...
	if !validChoice(borderStyle, "solid", "split", "split_top", "split_bottom", "blend", "neon", "spin", "dual") || borderSize < -1 || borderSize > 24 || spinSpeed < 10 || spinSpeed > 100 || !validChoice(shape, "native", "subtle", "soft", "rounded", "pill") || !validChoice(spacing, "native", "compact", "airy") || !validChoice(depth, "native", "flat", "shadow") || !validChoice(active, "native", "frosted_light", "frosted_balanced", "frosted_rich", "blur") || !validChoice(inactive, "native", "shadow", "shadow_only", "blur", "frosted_light", "frosted_balanced", "frosted_rich") {
		return fmt.Errorf("invalid desktop style")
	}
	if active == "blur" {
	...
	case "dual":
		accents := p.ActiveAccents()
		if len(accents) < 2 {
			accents = append(accents, p.Magenta)
		}
		activeBorder = evenBandGradientLua(hyprColors(accents), 45)
	}
...
func hyprColors(hex []string) []string {
	out := make([]string, len(hex))
	for i, h := range hex {
		out[i] = hyprColor(h)
	}
	return out
...
// Hyprland's border shader caps gradients at 10 stops (vec4 gradient[10]).
// Repeating each color widens its band without a palindrome, which would put
// the same color at both ends of the gradient axis and starve every other
// color to a thin seam.
func evenBandGradientLua(colors []string, angle int) string {
	repeat := 10 / len(colors)
	if repeat < 1 {
		repeat = 1
	}
	stops := make([]string, 0, len(colors)*repeat)
	for _, color := range colors {
		for i := 0; i < repeat; i++ {
			stops = append(stops, color)
		}
	}
	return gradientLuaColors(angle, stops...)
}
