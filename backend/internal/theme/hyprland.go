...
func WriteHyprland(themeDir string, p Palette, borderStyle string, borderSize int, shape, spacing, depth, inactive string, borderSpeed ...int) error {
	...
	if len(borderSpeed) > 0 && borderSpeed[0] != 0 {
		spinSpeed = borderSpeed[0]
	}
	if !validChoice(borderStyle, "solid", "split", "split_top", "split_bottom", "blend", "neon", "spin") || borderSize < -1 || borderSize > 24 || spinSpeed < 10 || spinSpeed > 100 || !validChoice(shape, "native", "subtle", "soft", "rounded", "pill") || !validChoice(spacing, "native", "compact", "airy") || !validChoice(depth, "native", "flat", "shadow") || !validChoice(inactive, "native", "shadow", "blur") {
	...
	if inactive == "blur" {
		...
		b.WriteString("hl.layer_rule({ name = \"omagen-live-canvas-backdrop-blur\", match = { namespace = \"^omagen-live-canvas$\" }, blur = true, blur_popups = true })\n")
	}
