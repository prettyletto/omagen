...
QtObject {
    ...
    function resume(data) { var next={}, paths={}; var variants=data.variants||[]; for (var i=0;i<variants.length;++i) { var e=variants[i]; if (e && e.variant && e.palette) { next[e.variant]=e.palette; paths[e.variant]=e.path||"" } } active=true; sessionId=data.session_id||""; var style=data.shell_style||data.desktop_style||({}); shellStyle={ surface: style.surface||"flat", detail: style.detail||"native", tooltip: style.tooltip||"native", notifications: style.notifications||"native" }; var bar=data.bar_style||({}); barStyle={ surface: bar.surface||"native", density: bar.density||"native", attention: bar.attention||"semantic", form: bar.form||"continuous", visibility: bar.visibility||"native" }; generationId=data.generation_id||""; palettes=next; variantPaths=paths; previewVariant=data.preview_variant||""; selectedVariant=previewVariant!=="" ? previewVariant : "source" }
    ...
    function clear() { active=false; sessionId=""; shellStyle=({ surface: "flat", detail: "native", tooltip: "native", notifications: "native" }); barStyle=({ surface: "native", density: "native", attention: "semantic", form: "continuous", visibility: "native" }); clearGeneration() }
}
