...
QtObject {
    ...
    function resume(data) { var next={}, paths={}; var variants=data.variants||[]; for (var i=0;i<variants.length;++i) { var e=variants[i]; if (e && e.variant && e.palette) { next[e.variant]=e.palette; paths[e.variant]=e.path||"" } } active=true; sessionId=data.session_id||""; originalTheme=data.original_theme||""; originalBackgroundKind=data.original_background_kind||""; originalBackgroundPath=data.original_background_path||""; var style=data.shell_style||data.desktop_style||({}); shellStyle={ surface: style.surface||"flat", detail: style.detail||"native" }; var bar=data.bar_style||({}); barStyle={ surface: bar.surface||"native", density: bar.density||"native", attention: bar.attention||"semantic" }; generationId=data.generation_id||""; palettes=next; variantPaths=paths; previewVariant=data.preview_variant||""; selectedVariant=previewVariant!=="" ? previewVariant : "source" }
    ...
    function clear() { active=false; sessionId=""; originalTheme=""; originalBackgroundKind=""; originalBackgroundPath=""; shellStyle=({ surface: "flat", detail: "native" }); barStyle=({ surface: "native", density: "native", attention: "semantic" }); clearGeneration() }
}
