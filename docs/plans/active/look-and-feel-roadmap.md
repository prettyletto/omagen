...
| 2. Composition persistence | Complete | Look & Feel and terminal metadata travel through begin/resume, regeneration, generation candidates, Live Canvas preview overrides, protocol checkpoints, and installed QML/backend transport; candidates carry `omagen.look-feel.json` and `omagen.terminal.json` | Terminal materialization, preset cards, terminal controls, live Glass proof |
