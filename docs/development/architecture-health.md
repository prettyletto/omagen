...
The audit was read-only. The checkout already contained unrelated edits before
the audit began; those edits were not reviewed as if they were audit changes
...
The highest-confidence maintenance candidates are small and behavior-neutral:
...
These are confirmed by repository search, not guesses about code that merely
looks old. Any cleanup should still be made in a separate change with focused
tests and a fresh full validation run.

### Unused CLI helpers

`parseRetintOptions` in
[`backend/internal/cli/preview_cmd.go`](../../backend/internal/cli/preview_cmd.go)
has no production caller. The active path uses `parseStudioOptions`, which
already handles the current `--run`, `--apps`, and `--skip` options plus scope,
wait, hook, color, and style options. Its remaining callers are tests.

`runSession` in
[`backend/internal/cli/session_cmd.go`](../../backend/internal/cli/session_cmd.go)
only forwards to `runSessionWithDependencies`; production dispatch calls the
dependency-aware function directly. The remaining callers are tests.
...
[`bin/__pycache__/omagen-file-selectcpython-314.pyc`](../../bin/__pycache__/omagen-file-selectcpython-314.pyc)
is tracked but is not referenced by the installer or runtime. The source
script, [`bin/omagen-file-select`](../../bin/omagen-file-select), is the runtime
input. Remove only the generated artifact in a dedicated cleanup change and
add `__pycache__/` and `*.pyc` to the repository ignore rules.
...
This is packaging maintenance rather than dead source code: the tests should
remain in the source checkout. Before excluding them from installation, add a
package-content assertion and verify that the development QML test runner still
finds them from the checkout.
...
The architecture checker currently reports:
...
- `backend/internal/cli/session_cmd.go` — about 402 lines.
