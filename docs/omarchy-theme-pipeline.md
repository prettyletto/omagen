...
Status: N1 inventory captured on 2026-08-22

This document records the installed Omarchy theme-set boundary used by Studio.
...
| `omagen.bar.toml` | Omagen `DockedBarSurface.qml` | Legacy Omagen Docked metadata |
...
| `omagen.bar.spec.json` | Omagen `DockedBarSurface.qml` and BarSpec compiler | Versioned surface/geometry/topology/behavior document; native continuous/minimal values reach `shell.toml`, while advanced values drive additive decoration without owning widget layout |
