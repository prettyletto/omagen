...
Item {
  ...
  // The Omagen bar uses this native source as its Default clone. The clone
  // keeps Quattro's own surface, geometry, ordering, drag, tray, tooltip, and
  ...
  function debugBarGeometry() {
    ...
    for (var i = 0; i < moduleSlots.length; i++) {
      ...
      out.push({
        ...
        itemVisible: !!slot.activeItem && slot.activeItem.visible === true,
        itemWidth: slot.activeItem ? Math.round(slot.activeItem.implicitWidth || 0) : 0,
        itemHeight: slot.activeItem ? Math.round(slot.activeItem.implicitHeight || 0) : 0,
        workspaceOverride: slot.workspaceOverride === true,
        registry: slot.registered === true,
        loaderStatus: slot.activeLoaderStatus,
        loaderError: slot.activeLoaderError,
        componentPresent: slot.registryComponent !== null,
        activeItemPresent: !!slot.activeItem,
        requestedTransparent: root.requestedTransparent === true,
        transparent: root.transparent === true,
        backgroundAlpha: root.background.a
      })
    }
    return out
  }
  ...
  onWorkspaceSpecOverrideChanged: {
    for (var i = 0; i < moduleSlots.length; i++) {
      var item = moduleSlots[i] && moduleSlots[i].activeItem
      if (item && "workspaceSpecOverride" in item) item.workspaceSpecOverride = workspaceSpecOverride
    }
  }
  ...
  component BarPanel: PanelWindow {
    ...
    // Keep the native center gesture as the primary handler, but retain a
    // full-surface fallback for empty bar pixels. Users can double-click the
    // background beside a widget and still reach Omarchy's transparency
    // toggle; child widget MouseAreas remain above this z:-1 fallback.
    MouseArea {
      id: fullSurfaceGesture
      anchors.fill: parent
      z: -1
  ...
  component ModuleSlot: Item {
    ...
    readonly property bool workspaceOverride: moduleName === "omarchy.workspaces"
      && root.workspaceOverrideEnabled === true
    ...
    Loader {
      id: workspaceLoader
      active: slot.workspaceOverride
      source: slot.workspaceOverride ? root.workspaceOverrideSource : ""
      anchors.fill: parent
      opacity: slot.dragSource ? 0.22 : 1.0
      onLoaded: {
        slot.injectProps()
      ...
      onStatusChanged: if (status === Loader.Error)
        console.warn("native bar clone workspace failed", errorString())
    }
